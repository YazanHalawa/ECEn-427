...
void XAC97_PlayAudio(Xuint32 BaseAddress, Xuint32 StartAddress,
	...
	else if (getVolLevel() == 1){
		//xil_printf("min\n\r");
		vol = AC97_VOL_MIN;
	}

	else if (getVolLevel() == 2){
		//xil_printf("mid\n\r");
		vol = AC97_VOL_MID;
	}
	else {
	...
	XAC97_WriteReg(BaseAddress, AC97_PCM_DAC_Rate, AC97_PCM_RATE_11025_HZ);

	/** Play Volume Settings **/
	XAC97_WriteReg(BaseAddress, AC97_MasterVol, AC97_VOL_MAX);
	XAC97_WriteReg(BaseAddress, AC97_AuxOutVol, vol);
	XAC97_WriteReg(BaseAddress, AC97_MasterVolMono, AC97_VOL_MAX);
	XAC97_WriteReg(BaseAddress, AC97_PCBeepVol, vol);
	XAC97_WriteReg(BaseAddress, AC97_PCMOutVol, vol);
	XAC97_WriteReg(BaseAddress, AC97_LineInVol, AC97_VOL_MAX);
	XAC97_WriteReg(BaseAddress, AC97_MicVol, AC97_VOL_MAX);
	...
	while( sound_ptr < (Xuint32*)EndAddress) {
		...
		XAC97_mSetInFifoData(BaseAddress, sample | sample << 16);
	}
